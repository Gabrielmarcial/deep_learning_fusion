...
# Only rotations by 90 degrees for data generation
...
def train_net(net, x_train, y_train, x_valid, y_valid, batch_size, epochs, early_stopping_epochs, early_stopping_delta, filepath, filename, data_augmentation=False, number_samples_for_generator=1, history_file = None):
    ...
    for epoch in range(epochs):
        ...
        show_graph_loss_accuracy(np.asarray([ history_train, history_valid ]),1,root_path+f'train_{epoch}_history.png')
    ...
    # Save and/or return history updated
    history = [ history_train, history_valid ]
    if history_file:
        updated = save_history(history_file, history, True)
...
if __name__ == '__main__':
    ...
    history = train_net(model, x_train[0:64], y_train[0:64], x_valid[0:16], y_valid[0:16], batch_size, epochs, early_stopping_epochs, early_stopping_delta, root_path, best_model_filename, data_augmentation, number_samples_for_generator, "../history.json")
