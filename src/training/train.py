...
def get_state(loss_arr, delta):
    best_offset = 0
    best_vloss = float('inf')
    for i in range(len(loss_arr)):
        if (1-(loss_arr[i]/best_vloss)) < delta:
            best_offset += 1
        else:
            best_vloss, best_offset = loss_arr[i], 0
    return best_vloss, best_offset
...
def train_net(net, x_train, y_train, x_valid, y_valid, batch_size, epochs, early_stopping_epochs, early_stopping_delta, filepath, filename, data_augmentation=False, number_samples_for_generator=1, history_file = None):
    ...
    # restore history is available
    history = [[],[]]
    if history_file:
        data = load_history(history_file)
        valid_loss_best_model, no_improvement_count = get_state(data['vloss'], early_stopping_delta)
        history = decode_hist(data)
    ...
    for epoch in range(epochs):
        ...
        if (1-(valid_loss[0 , 0]/valid_loss_best_model)) < early_stopping_delta:
            if no_improvement_count+1 >= early_stopping_epochs:
                ...
                del net
                gc.collect()
                net = tf.keras.models.load_model('temp.keras', compile=True)
